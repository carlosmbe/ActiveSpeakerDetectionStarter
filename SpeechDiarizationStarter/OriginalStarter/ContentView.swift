...
struct ContentView: View {
    ...
    @ObservedObject private var TranscribeViewModel = Transcriber()
    @State private var transResults = "Result of Transcribe Will Go Here"
   ...
   // @State private var mediaURL: URL? = Bundle.main.url(forResource: "Clip", withExtension: "mp4")
    ...
    var body: some View {
        VStack(spacing: 10) {
            ...
            Button("Run Transcribe on Converted Audio") {
                Task{
                    if let convertedAudioURL{
                        let fileName = convertedAudioURL.deletingPathExtension().lastPathComponent
                         try? await TranscribeViewModel.performSpeechRecognition(audioURL: convertedAudioURL)
        ...
        .onChange(of: TranscribeViewModel.recognizedUtterances.count) { newValue in
            for i in TranscribeViewModel.recognizedUtterances{
